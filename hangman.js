...
var cmds = {
	hangmanhelp: function(target, room, user) {
		...
		this.sendReplyBox('<font size = 2>Una pequeña introducción al </font><font size = 3>Hangman:</font><br />' +
						...
						'<li>/category [descripción] O /topic [descripción] - Permite a la persona que inició el juego cambiar la descripción.</li>' +
						'<li>/endhangman - Finaliza el juego de hangman. Requiere: + % @ & ~</li></ul>' +
						'Debido a cambios recientes, hangman puede ser jugado en multiples salas a la vez exepto el lobby, por ser demasiado grande.<br />' +
						'Diviertete y sientete libre de mandar PM si encuentras algun error en el sistema - Alchimist');					
	},
	...
	hangman: function(target, room, user) {
		...
		if(room.type === 'battle') {
			return this.sendReply('No puedes iniciar un juego de hangman durante una batalla.');
		}
		if(hangman[room.id].hangman === false) {
			var targets = target.split(',');
			if(!targets[1]) {
				return this.sendReply('Debes incluir una descripción');
			}
			if(targets[0].length > 10) {
				return this.sendReply('La palabra no puede ser mayor a 10 letras.')
			}
	...
	viewhangman: function(target, room, user) {
		if(room.id === 'lobby') {
				return this.sendReply('|html|Por favor juega en otra parte. Es demasiado grande para el lobby.');
		}
		if (!this.canBroadcast()) return;
		if(hangman[room.id].hangman === false) {
	...
	guess: function(target, room, user) {
		...
		if(user.userid === hangman[room.id].hangmaner[0]) {
			return this.sendReply('No puedes adivinar porque tu iniciaste el juego.');
		}
		if(!target) {
