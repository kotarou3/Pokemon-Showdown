...
exports.commands = {
	...
	clanauth: function (target, room, user) {
		...
		if (!clan) {
			...
			if (target) clan = Clans.getRating(target);
		}
		if (!clan && autoclan) {
	...
	getclan: function (target, room, user) {
		...
		if (!clan) {
			...
			if (target) clan = Clans.getProfile(target);
		}
		if (!clan && autoClan) {
			target = Clans.findClanFromMember(user.name);
			if (target) clan = Clans.getProfile(target);
	...
	oficialclan: function (target, room, user) {
		...
		let perminsionValue = 0;
		if (clanUser) {
			let clanUserid = toId(clanUser);
			let iduserwrit = toId(user.name);
			perminsionValue = Clans.authMember(clanUserid, iduserwrit);
