...
exports.getTours = function () {
	if (!wars) return 'No hay ninguna guerra en curso.';
	let tourList = '';
	for (let w in wars) {
	...
	if (!tourList || tourList === '') return 'No hay ninguna guerra en curso.';
	return tourList;
};
...
exports.getFreePlaces = function (room) {
	let roomId = toId(room);
	if (!wars[roomId]) return 0;
	let membersA = wars[roomId].size;
	let membersB = wars[roomId].size;
...
exports.joinable = function (room, user) {
	let roomId = toId(room);
	let userId = toId(user);
	let playersA = wars[roomId].teamAMembers;
	let playersB = wars[roomId].teamBMembers;
...
exports.startTeamTour = function (room) {
	...
	if (wars[roomId].type === 'lineups') {
		teamAMembers = exports.getAvailableMembers(wars[roomId].teamAMembers);
		teamBMembers = exports.getAvailableMembers(wars[roomId].teamBMembers);
	} else {
		teamAMembers = exports.getAvailableMembers(wars[roomId].teamAMembers).randomize();
		teamBMembers = exports.getAvailableMembers(wars[roomId].teamBMembers).randomize();
	}
...
exports.newRound = function (room) {
	...
	for (let m in wars[roomId].matchups) {
...
exports.autoEnd = function (room) {
	...
	let nMatchups = 0;
	let nByes = 0;
	for (let b in wars[roomId].matchups) {
		++nMatchups;
...
exports.replaceParticipant = function (room, p1, p2) {
	...
	if (toId(wars[roomId].matchups[matchupId].from) === toId(p1)) {
		wars[roomId].matchups[matchupId].from = p2;
	} else if (toId(wars[roomId].matchups[matchupId].to) === toId(p1)) {
		wars[roomId].matchups[matchupId].to = p2;
	}
	return false;
};
...
exports.viewTourStatus = function (room) {
	...
	if (wars[roomId].tourRound === 0) {
		...
		for (let i in wars[roomId].matchups) {
			let userk = Users.getExact(wars[roomId].matchups[i].from);
			if (!userk) {
				userk = wars[roomId].matchups[i].from;
			} else {
				userk = userk.name;
			}
			let userf = Users.getExact(wars[roomId].matchups[i].to);
			if (!userf) {
				userf = wars[roomId].matchups[i].to;
			} else {
				userf = userf.name;
			}
...
let cmds = {
	...
	war: function (target, room, user, connection) {
		...
		case 'replace': {
			...
			if (clanUser && (Clans.authMember(clanUser, user.name) === 1 || Clans.authMember(clanUser, user.name) === 2 || Clans.authMember(clanUser, user.name) === 3)) canReplace = true;
			if (!canReplace && !this.can('tournamentsmoderation', room)) return false;
			let clanReplace = Clans.findClanFromMember(params[2]);
...
Rooms.global.startBattle = function (p1, p2, format, rated, p1team, p2team) {
	let newRoom = this.___startBattle(p1, p2, format, rated, p1team, p2team);
	if (!newRoom) return;
	//tour
	let matchup = War.findTourFromMatchup(p1.name, p2.name, format, newRoom.id);
	if (matchup) {
		newRoom.war = 1;
...
Rooms.BattleRoom.prototype.win = function (winner) {
	//tour
	if (this.war) {
		let matchup = War.findTourFromMatchup(this.p1.name, this.p2.name, this.format, this.id);
		if (matchup) {
			let losser = false;
